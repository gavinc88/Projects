...
void execute_one_inst(processor_t* p, int prompt, int print_regs)
{
  ...
  case 0x0: // opcode == 0x0 (SPECIAL)
	...
	case 0x9: // opcode == 0x9 (addiu)
		p->R[inst.itype.rt] = p->R[inst.itype.rs] + inst.chunks16[0];
		p->pc += 4;
		break;
	
	case 0xa: // opcode == 0xa (slti)
		p->R[inst.itype.rt] = (signed)p->R[inst.itype.rs] < inst.chunks16[0];
		p->pc += 4;
		break;
	...
	case 0x20: // opcode == 0x20 (lb)
		p->R[inst.itype.rt] = (int8_t)(load_mem(p->R[inst.itype.rs] + inst.chunks16[0], SIZE_BYTE));
		p->pc += 4;
		break;
		
	case 0x21: // opcode == 0x21 (lh)
		p->R[inst.itype.rt] = (int16_t)(load_mem(p->R[inst.itype.rs] + inst.chunks16[0], SIZE_HALF_WORD));
		p->pc += 4;
		break;
		
	case 0x23: // opcode == 0x23 (lw)
		p->R[inst.itype.rt] = load_mem(p->R[inst.itype.rs] + inst.chunks16[0], SIZE_WORD);
		p->pc += 4;
		break;
		
	case 0x24: // opcode == 0x24 (lbu)
		p->R[inst.itype.rt] = load_mem(p->R[inst.itype.rs] + inst.chunks16[0], SIZE_BYTE);
		p->pc += 4; 
		break;
		
	case 0x25: // opcode == 0x25 (lhu)
		p->R[inst.itype.rt] = load_mem(p->R[inst.itype.rs] + inst.chunks16[0], SIZE_HALF_WORD);
		p->pc += 4; 
		break;
		
	case 0x28: // opcode == 0x28 (sb)
		store_mem(p->R[inst.itype.rs] + inst.chunks16[0], SIZE_BYTE, p->R[inst.itype.rt]);
		p->pc += 4; 
		break;
		
	case 0x29: // opcode == 0x29 (sh)
		store_mem(p->R[inst.itype.rs] + inst.chunks16[0], SIZE_HALF_WORD, p->R[inst.itype.rt]);
		p->pc += 4; 
		break;
		
	case 0x2b: // opcode == 0x2b (sw)
		store_mem(p->R[inst.itype.rs] + inst.chunks16[0], SIZE_WORD, p->R[inst.itype.rt]);
		p->pc += 4; 
		break;
